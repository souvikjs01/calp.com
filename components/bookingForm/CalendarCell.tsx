  ...
  return (
    <td {...cellProps} className={`py-0.5 px-0.5 relative ${isFocusVisible ? 'z-10' : 'z-0'}`}>
      <div
        ...
        className={`size-8 sm:size-10 outline-none group rounded-md`}
      >
        <div className={cn(
            ...
            !isSelected && !finalIsDisabled ? 'bg-primary/10' : ''
        )}>
            {formattedDate}
            {isDateToday && (
              <div className={cn(`absolute bottom-3 left-1/2 transform -translate-x-1/2
               translate-y-1/2 size-1.5 bg-primary rounded-full`, isSelected && 'bg-white')}></div>
            )}
        </div>
      </div>
    </td>
